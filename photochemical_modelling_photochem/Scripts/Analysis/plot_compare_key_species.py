...
def main():
    parser = argparse.ArgumentParser(description="Plot key species comparison with pressure oriented correctly.")
    parser.add_argument("--output", default=r"output/figures/Earth_A1_key_species_comparison.png")
    parser.add_argument(
        "--official-output",
    ...
    fig, axes = plt.subplots(1, len(species), figsize=(19, 5.8), sharey=True)
    if len(species) == 1:
        axes = [axes]

    pressures = [old_data[:, 0], new_data[:, 0]]
    if official_loaded:
        pressures.append(official_data[:, 0])
    max_pressure = max(arr.max() for arr in pressures)
    min_pressure = min(arr.min() for arr in pressures)

    for ax, sp in zip(axes, species):
        ax.plot(old_data[:, old_idx[sp]], old_data[:, 0], label="Original", lw=2.4, color="#1f77b4")
        ax.plot(new_data[:, new_idx[sp]], new_data[:, 0], label="Photochem A1", lw=2.2, color="#d62728", ls="--")
        if official_loaded and sp in official_idx:
            ax.plot(official_data[:, official_idx[sp]], official_data[:, 0], label="Photochem official", lw=2.2, color="#2ca02c", ls=":")
        ax.set_xscale("log")
        ax.set_yscale("log")
